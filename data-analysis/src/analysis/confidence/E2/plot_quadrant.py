...
MODEL_GROUP_BOUNDARY_1 = 2.6
MODEL_GROUP_BOUNDARY_2 = 3.0
...
def add_model_group(
    ...
    """
    Regroupe les participants selon une colonne de nombre moyen
    de modèles.
    ...
    if models_column not in dataframe.columns:
        raise KeyError(
            "Colonne de modèles absente : "
            f"{models_column}"
        )

    dataframe[output_column] = pd.cut(
        dataframe[models_column],
        bins=[
            -np.inf,
            MODEL_GROUP_BOUNDARY_1,
            MODEL_GROUP_BOUNDARY_2,
            np.inf,
        ],
...
def build_subject_summary(dataframe):
    ...
    subject_summary = add_model_group(
        dataframe=subject_summary,
        models_column=(
            "mean_number_models_generated_int"
        ),
        output_column="model_group_int",
    )
...
def create_grouped_medians_plot(dataframe):
    ...
    Chaque participant apparaît donc une seule fois dans le graphique.
    ...
    for axis, group_label in zip(
        ...
        axis.scatter(
            ...
            group_data["accuracy"],
            s=75,
            color=MODEL_GROUP_COLORS[group_label],
            alpha=0.65,
            edgecolors="white",
            linewidths=0.7,
            label="Participants",
