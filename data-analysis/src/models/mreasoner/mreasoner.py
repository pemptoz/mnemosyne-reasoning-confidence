...
def source_path(mreas_path= None):
...
class MReasoner():
    ...
    def query_trace(self, premises, param_dict=None):
        """
        Envoie une requête syllogistique au processus LISP et récupère
        la trace complète de l'exécution pour analyser la construction des modèles.
        """
        if param_dict == None:
            param_dict = DEFAULT_PARAMS

        # L'astuce ici est d'utiliser format t pour délimiter clairement la trace
        # dans la sortie standard (stdout) avec ---TRACE_START--- et ---TRACE_END---
        cmd = [
            "(progn",
            "    (initialize-tracer)",
            "    (reset-tracer)",
            ...
            "        (setf +omega+ {})".format(param_dict['omega']),
            "        (what-follows? premise-intensions)",
            "        (format t \"~%---TRACE_START---~%~S~%---TRACE_END---~%\" (trace-output *tracer*))",
            "    )",
            ")",
        ]
        ...
        # On attend la réponse multilignes
        out = self.wait_for_trace_output(timeout=15)

        # Catch timeout (sécurité anti-crash LISP)
        if out is None:
            self.logger.warn(
                'Timeout in query_trace detected for "%s" with params "%s"', premises, param_dict)
            self.terminate()
            self.initialize()
            return self.query_trace(premises, param_dict=param_dict)

        return out

    def wait_for_trace_output(self, timeout=15):
        """
        Écoute de manière asynchrone la sortie standard (stdout) du processus LISP
        pour capturer toutes les lignes entre les balises START et END.
        """
        manager = multiprocessing.Manager()
        result_list = manager.list()

        def worker(proc):
            logger = logging.getLogger(__name__ + '-trace-worker')
            recording = False
            lines = []
            while True:
                # Lecture non bloquante avec nettoyage des caractères
                line = proc.stdout.readline().decode('ascii', errors='ignore').strip()
                
                if '---TRACE_START---' in line:
                    recording = True
                elif '---TRACE_END---' in line:
                    # Fin de la trace, on sauvegarde et on quitte le thread
                    result_list.append('\n'.join(lines))
                    return
                elif recording:
                    lines.append(line)
        ...
        if th.is_alive():
            self.logger.warn('Timeout occurred in trace worker.')
            th.terminate()
            return None
