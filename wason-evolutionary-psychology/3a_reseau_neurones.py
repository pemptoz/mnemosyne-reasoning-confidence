"""moyen avec optimisation classique"""
import torch
...
def verification(situation, decision, is_social):
    ...
    if is_social:
        # CONTEXTE SOCIAL : Détecter un tricheur est crucial
        if situation == 0 or situation == 3: 
            if random.random() < 0.2: 
                if decision == 1: reward += 10  # Grosse récompense de détection
                else: reward -= 7               # Grosse punition si on se fait avoir
    else:
        # CONTEXTE ABSTRAIT : Aucune menace, juste de la curiosité logique
        if situation == 0 or situation == 3:
            if random.random() < 0.2:
                if decision == 1: reward += 1   # Petite satisfaction (+1), ne compense pas l'effort (-1)
    return reward
